...
function removeNodeToClient(id) {
  // Find node, remove it and disconnect
  for(let i = 0; i < clientState.groups.length; i++) {
    for(let j = 0; j < clientState.groups[i].chunks.length; j++) {
      if(clientState.groups[i].chunks[j].id === id) {
      }
    }
  }
}

function updateNodeToClient(node) {
  ...
  if (
    ...
    console.log("Invalid call to updateNodeToClient, missing id, group, or label");
    return;
  }
  // Convert to hex string if number
  if (typeof node.id === "number") node.id = node.id.toString(16);
...
function addNodeToClient(node) {
  ...
  console.log(
    ...
    groupIndex,
    "group:",
    clientState.groups[groupIndex].name
  );
  ...
  console.log(
    ...
    nodeIndex,
    "size of chunks",
    clientState.groups[groupIndex].chunks.length
  );
  if (nodeIndex === 0) {
    ...
    web.emit("connect-nodes", {
      from: node.id,
      to: clientState.groups[groupIndex].chunks[1].id
    });
    return;
  } else if (nodeIndex === clientState.groups[groupIndex].chunks.length) {
    console.log(
      "Case 2, === clientState.groups[groupIndex].chunks.length:",
      clientState.groups[groupIndex].chunks.length
    );
                ...
                derefAddr(sk, heap_base_addr).then(heap_base => {
                  getTcacheBins(sk, heap_base).then(tcache_bins => {
                    ...
                    if (gTcache.data.bins.includes(freedAddr)) {
                      ...
                      var tCacheGroup = state.groups.find(
                        g => g.name === "tcache"
                      );
                      var newChunk = condense(
                        freedAddr,
